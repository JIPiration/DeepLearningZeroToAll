...
class LeafModel(tez.Model):
    ...
    def __init__(self, num_classes):
        ...
        self.effent = EfficientNet.from_pretrained("efficientnet-b3")
    ...
    def fetch_scheduler(self):
        sch = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(
            self.optimizer, T_0=10, T_mult=1, etc_min=1e-6, last_epoch=1
        )
        return sch
...
if __name__ == "__main__":
    ...
    TEST = False
    if TEST:
        test_dfx = pd.read_csv(os.path.join(INPUT_PATH, "sample_submission.csv"))
        test_image_paths = [os.path.join(TEST_IMAGE_PATH, x) for x in test_dfx.image_id.values]
        # fack targets
