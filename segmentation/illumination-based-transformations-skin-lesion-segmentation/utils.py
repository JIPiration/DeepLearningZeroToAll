...
theta = torch.FloatTensor([PI / 180 * (idx + 1) for idx in range(100)])
...
def entropy_intrinsic(image, calculate_intrinsic_img=False):
    ...
    if calculate_intrinsic_img:
        projected = torch.matmul(ch1, XY[:, int(np.argmin(tmp_entropy))]).reshape(128, 128)
